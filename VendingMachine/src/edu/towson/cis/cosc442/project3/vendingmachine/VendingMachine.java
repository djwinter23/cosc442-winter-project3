...
public class VendingMachine {
	...
	private double balance; //should be private and therefore unchangeable to another class outside of VendingMachine
	...
	public VendingMachineItem removeItem(String code) throws VendingMachineException {
		int slotIndex = getSlotIndex(code);
		VendingMachineItem item = itemArray[slotIndex];//could be getItem(code)?
		if ( item == null) {
			throw new VendingMachineException(SLOT_MESSAGE + code + IS_EMPTY_MESSAGE);
		}
		else 
			itemArray[slotIndex] = null;//bug: should check first before removing?
		return item;
	}
	...
	public boolean makePurchase(String code) {
		...
		VendingMachineItem item = getItem(code);//itemArray[]
